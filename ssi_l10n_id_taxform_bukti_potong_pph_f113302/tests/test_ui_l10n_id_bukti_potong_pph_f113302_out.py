...
#: limit.
_TOUR_PREFIX = (
    "ssi_l10n_id_taxform_bukti_potong_pph_f113302_"
    "l10n_id_bukti_potong_pph_f113302_out_"
)


@tagged("post_install", "-at_install")
class TestUiL10nIdBuktiPotongPphF113302Out(HttpSavepointCase):
    """Tour tests for the ``l10n_id.bukti_potong_pph_f113302_out`` work
    instructions."""
    ...
    def setUpClass(cls):
        ...
        cls.journal = cls.env.ref(
            "ssi_l10n_id_taxform_bukti_potong_pph_f113302.demo_journal1"
        )
        cls.account = cls.env.ref(
            "ssi_l10n_id_taxform_bukti_potong_pph_f113302.demo_account1"
        )
        cls.tax = cls.env.ref("ssi_l10n_id_taxform_bukti_potong_pph_f113302.demo_tax1")
        cls.expense_account = cls.env["account.account"].create(
            {
                "name": "TOUR Expense F113302",
                "code": "XTOURF113302EXP",
        ...
        cls.tax_period = cls.env["l10n_id.tax_period"].create(
            {
                "name": "TOUR 06/2026 F113302",
                "code": "TOUR-06/2026-F113302",
        ...
        cls.ttd = cls.env["res.partner"].create(
            {
                "name": "TOUR TTD F113302",
                "is_company": False,
                "parent_id": cls.company.partner_id.id,
            }
        )

        cls.wajib_pajak_create = cls._create_wajib_pajak("TOUR WP CreateF113302")
        cls.wajib_pajak_confirm = cls._create_wajib_pajak("TOUR WP ConfirmF113302")
        cls.wajib_pajak_approve = cls._create_wajib_pajak("TOUR WP ApproveF113302")
        cls.wajib_pajak_cancel = cls._create_wajib_pajak("TOUR WP CancelF113302")

        cls.cancel_reason = cls.env["base.cancel_reason"].create(
            {
                "name": "TOUR Cancel Reason F113302",
                "code": "TOURXF113302",
                "global_use": True,
            }
        )
        ...
        cls.env["l10n_id.bukti_potong_pph_f113302_out_line"].create(
            {
    ...
    def _create_income_move_line(cls, partner):
        ...
        The returned line stands in for the "income" (payable) entry
        that a withholding line references through ``move_line_id``:
        it must belong to a **posted** move so ``_create_aml`` can
        later reconcile the withholding line's debit against it.

        :param partner: partner on the credit line
        ...
        move = cls.env["account.move"].create(
            {
                ...
                "line_ids": [
                    ...
                    (
                        ...
                        0,
                        {
                            "name": "TOUR Income Line F113302 Contra",
    ...
    def _create_order(cls, wajib_pajak):
        """Create a draft Bukti Potong PPh f.1.1.33.02 Out document.
        ...
        :return: the created ``l10n_id.bukti_potong_pph_f113302_out``
            record
        """
        return cls.env["l10n_id.bukti_potong_pph_f113302_out"].create(
            {
    ...
    def test_create(self):
        """Run the create tour for Bukti Potong PPh f.1.1.33.02 Out.
    ...
    def test_confirm(self):
        """Run the confirm tour for Bukti Potong PPh f.1.1.33.02 Out.

        IK: docs/l10n_id_bukti_potong_pph_f113302_out/04-confirm.md
        """
        self.start_tour(
            "/web",
    ...
    def test_approve(self):
        """Run the approve tour for Bukti Potong PPh f.1.1.33.02 Out.

        IK: docs/l10n_id_bukti_potong_pph_f113302_out/05-approve.md
        """
        self.start_tour(
            "/web",
    ...
    def test_cancel(self):
        """Run the cancel tour for Bukti Potong PPh f.1.1.33.02 Out.

        IK: docs/l10n_id_bukti_potong_pph_f113302_out/10-cancel.md
        """
        self.start_tour(
            "/web",
            _TOUR_PREFIX + "cancel",
