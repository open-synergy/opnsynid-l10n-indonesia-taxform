...
class TestUiL10nIdBuktiPotongPphF113308OutOperatingUnit(HttpSavepointCase):
    """UI/UX tour test for the Operating Unit field this module adds to
    ``l10n_id.bukti_potong_pph_f113308_out`` (E1 delta -- Additional
    Fields).

    Pairs with IK ``docs/l10n_id_bukti_potong_pph_f113308_out/
    01-create.md`` (delta), backed by the base IK
    ``ssi_l10n_id_taxform_bukti_potong_pph_f113308/docs/
    l10n_id_bukti_potong_pph_f113308_out/01-create.md`` for the
    ...
    def setUpClass(cls):
        ...
        cls.operating_unit_partner = cls.env["res.partner"].create(
            {"name": "TOUR Bukti Potong F113308 Out OU Partner"}
        )
        cls.operating_unit = cls.env["operating.unit"].create(
            {
                "name": "TOUR Bukti Potong F113308 Out Operating Unit",
                "code": "TBP38OOU",
