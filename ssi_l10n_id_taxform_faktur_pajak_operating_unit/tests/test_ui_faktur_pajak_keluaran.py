...
class TestUiFakturPajakKeluaran(HttpSavepointCase):
    ...
    def setUpClass(cls):
        ...
        operating_unit_partner = cls.env["res.partner"].create(
            {"name": "Tour Faktur Pajak Keluaran OU Partner"}
        )
        cls.operating_unit = cls.env["operating.unit"].create(
            {
                "name": "Tour Faktur Pajak Keluaran Operating Unit",
                "code": "TFPKOU",
    ...
    def test_create(self):
        """Run the create tour for ``faktur_pajak_keluaran``.
